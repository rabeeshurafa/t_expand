...
class TaboFinalImport implements ToCollection, WithValidation, WithHeadingRow
{
    protected $excel_id;
    ...
    public function collection(Collection $collection)
    {
        foreach ($collection as $item) {
            ...
            $taboData=TaboData::where('excel_id',$this->excel_id)->where('temp_no',$itemArray[0])->first();
            if($taboData==null){
                $taboData=new TaboData();
            }
            $taboData->excel_id =$this->excel_id;
            $taboData->temp_no =$itemArray[0];
            $taboData->final_no =$itemArray[3];
