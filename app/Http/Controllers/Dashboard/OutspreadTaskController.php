...
class OutspreadTaskController extends Controller
{
    ...
    public function trashTasks()
    {
        ...
        $app_no = 23;
        return view('dashboard.outspreadTasks.trashTasks',
                compact('ticket_status', 'type', 'ticketTypeList', 'region', 'ticketInfo', 'department', 'fees',
    ...
    public function outspreadTasks()
    {
        ...
        $ticket_status = Constant::where('parent', 5001)->where('status', 1)->get();
        $app_no = 23;
        return view('dashboard.outspreadTasks.outspreadTask',
                compact('ticket_status', 'type', 'ticketTypeList', 'region', 'ticketInfo', 'department', 'fees',
                        'archive_config', 'app_no'));
    }

    public function wasteComplaint()
    {
        $ticketTypeList = Constant::where('parent', 6405)->where('status', 1)->get();
        $setting = Setting::first();
        $region = Region::where('town_id', $setting->town_id)->get();
        $type = 'wasteComplaint';
        $ticketInfo = $this->loadDefaul($type);
        ...
        $ticket_status = Constant::where('parent', 5001)->where('status', 1)->get();
        $app_no = 43;
        return view('dashboard.outspreadTasks.wasteComplaint',
                compact('type', 'ticketTypeList', 'region', 'ticketInfo', 'department', 'fees', 'archive_config',
                        'app_no', 'ticket_status'));
    }
    ...
    public function quittance()
    {
        ...
        $archive_config = ArchiveRole::where('empid', Auth()->user()->id)->where('type', $type)->get();
        $app_no = 27;
        return view('dashboard.outspreadTasks.quittance',
                compact('type', 'resonTypeList', 'ticketInfo', 'department', 'fees', 'archive_config', 'app_no'));
    }
    ...
    public function innerQuittance()
    {
        ...
        $archive_config = ArchiveRole::where('empid', Auth()->user()->id)->where('type', $type)->get();
        $app_no = 36;
        return view('dashboard.outspreadTasks.innerQuittance',
                compact('type', 'resonTypeList', 'ticketInfo', 'department', 'fees', 'archive_config', 'app_no'));
    }
    ...
    public function publicComplaint()
    {
        ...
        $app_no = 24;
        return view('dashboard.outspreadTasks.publicComplaint',
                compact('type', 'ticketInfo', 'department', 'fees', 'archive_config', 'app_no', 'complaintTypes'));
    }
    ...
    public function citizenComplaint()
    {
        ...
        $archive_config = ArchiveRole::where('empid', Auth()->user()->id)->where('type', $type)->get();
        $app_no = 25;
        return view('dashboard.outspreadTasks.citizenComplaint',
                compact('type', 'ticketInfo', 'department', 'fees', 'archive_config', 'app_no'));
    }
}
