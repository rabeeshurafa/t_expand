...
class DashboardController extends Controller
{
    ...
    public function getArchiveInfo()
    {
        ...
        $tradeArchive = TradeArchive::where('enabled', 1)
            ->select(FacadesDB::raw("SUM(`sizeAttachments`) as sizeAttachments"))->first();
        $licArchive = ArchiveLicense::where('enabled', 1)
            ->select(FacadesDB::raw("SUM(`sizeAttachments`) as sizeAttachments"))->first();
        $sum = 0;
        if ($archive_max) {
        ...
        if ($tradeArchive) {
            $sum += $tradeArchive->sizeAttachments;
        }
        if ($licArchive) {
            $sum += $licArchive->sizeAttachments;
        }
        $archive['archive_max'] = $archive_max;
    ...
    {
        $activeRec = $this->indexMasterQuery(
            " where app_trans.id = a.active_trans and app_trans.ticket_type = 2 ",
            1,
            11
        );
        $res = DB::select("SELECT app_trans.*,admins.nick_name,admins.image FROM `app_trans` join admins on app_trans.sender_id=admins.id WHERE `reciver_id`=" . Auth()->user()->id . " and EXISTS ($activeRec) order by id desc");
