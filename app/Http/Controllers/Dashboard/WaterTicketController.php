...
class WaterTicketController extends Controller
{
    ...
    public function appCustomer(Request $request){
        ...
        $valid=1;
        $errorList=array();
        $names = User::where('id',$request['id'] )->where('enabled',1 )->select('*')->with('jobLic')->with('License')->get();
        $warnings=Warning::where('warning_status',6240)->where('enabled',1)->where('subscriber_id',$request['id'])->with('type')->with('addBy')->get();
    ...
    public function waterPermission()
    {
        ...
        $archive_config = ArchiveRole::where('empid', Auth()->user()->id)->where('type', $type)->get();
        $app_no=3;
        return view('dashboard.water_ticket.appTicket3', compact('type','region','ticketInfo','department','app_type','subsList','fees','archive_config','app_no','converterNos'));
    }
