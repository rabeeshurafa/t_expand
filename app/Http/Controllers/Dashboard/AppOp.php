...
class AppOp extends Controller
{
    ...
    function dowaivesubscription(Request $request){
        ...
        if($subs != null){
            return response()->json(['subs'=>$subs,'app_id'=>$ticket->id,'app_type'=>$request->related,'success'=>trans('تم الحفظ')]);
        }else{
...
order by created_at asc"); 
    ...
    function saveReplyScannedFilesArchieve(Request $request,$taskname='',$tasklink='',$model='App\\Models\\User'){
        $files_ids = $request->attach_ids;
	...
	public function saveTicket1(TicketRequest $request){
	   ...
	   // dd($this->prepeardebt($request));
        ...
        if(!$app_id){
            ...
            $app->receipt_no	=$request->ReciptNo;
            if($request->phase != null)             $app->phase	        =$request->phase[0];	
            $app->inAmper	    =$request->inAmper;
            $app->amount		=$request->AmountInNIS;
            $app->currency		=$request->CurrencyID;
            $app->subscription_type	=$request->subscriptionType;	
    ...
    public function saveTicket3(TicketRequest22 $request){
	   // dd($request->all());
        ...
        if(!$app_id){
            ...
            $maxRec=AppTicket3::select('app_no')->where('app_type',$request->app_type)->orderBy('id','desc')->limit(1)->get();
            $max=1;
    ...
    public function saveTicket39(Ticket39Request $request){
        ...
        if(!$app_id){
            $maxRec=AppTicket39::select('app_no')->where('app_type',$request->app_type)->orderBy('id','desc')->limit(1)->get();
            $max=1;
    ...
    public function saveTicket40(Ticket40Request $request){
        ...
        if(!$app_id){
            $maxRec=AppTicket40::select('app_no')->where('app_type',$request->app_type)->orderBy('id','desc')->limit(1)->get();
            $max=1;
