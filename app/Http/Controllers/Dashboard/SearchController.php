...
class SearchController extends Controller
{
        ...
        public function full_search(Request $request)
        {
                ...
                $names = $equip->merge($vehicle)->merge($project)
                        ...
                        /*->merge($archiveLicense)*/->merge($user)->merge($folder)->merge($userNationalID)->merge($Volunteer);

                return response()->json($names);
        }
        public function full_search_email(Request $request)
        {
                $emp_data = $request['term'];
                $admin = Admin::where('admins.id', '!=', '74')
                        ->where('email', 'like', '%' . $emp_data . '%')->where('enabled', 1)
                        ->select('*', DB::raw("email AS label"))->get();
                $department = Department::where('email', 'like', '%' . $emp_data . '%')
                        ->where('enabled', 1)
                        ->select('*', DB::raw("email AS label"))->get();
                $orgnization = Orgnization::where('email', 'like', '%' . $emp_data . '%')
                        ->where('enabled', 1)
                        ->select('*', DB::raw("email AS label"))->get();
                $user = User::where('email', 'like', '%' . $emp_data . '%')
                        ->where('enabled', 1)
                        ->select('*', DB::raw("email AS label"))->get();
                $email = $admin->merge($department)->merge($orgnization)->merge($user);
