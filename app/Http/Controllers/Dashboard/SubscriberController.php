        ...
        if ($request->subscriber_id == null) {
            ...
            if ($national_id != null && $request->formDataNationalID != null) {
                return response()->json([
                        'status' => false, 'errors' => array('national_id' => array('رقم الهوية مكرر'))
                ]);
            }

        } else {

            $national_id = User::where('national_id', $request->formDataNationalID)->where('enabled', '1')->where('id',
                    '!=', $request->subscriber_id)->first();
            if ($national_id != null && $request->formDataNationalID != null) {
                return response()->json([
                        'status' => false, 'errors' => array('national_id' => array('رقم الهوية مكرر'))
                ]);
...
//            $user->passport_number = $request->passport_number;
    ...
    public function subscribe_auto_complete(Request $request)
    {
        ...
        $names = User::where('name', 'like', '%'.$subscriber_data.'%')->where('enabled', 1)->select('*',
                'name as label')->get();
    ...
    public function subscribe_delete(Request $request)
    {
        ...
        $jobLicArchieves = jobLicArchieve::where('model_id', $user->id)->where('model_name',
                'App\\Models\\User')->get();
    ...
    public function getUserData(Request $request)
    {
        ...
        $user['otherArchiveCount'] = Archive::where('model_id', $request['subscribe_id'])->where('enabled', 1)
                ->where('model_name', $model)->whereNotIn('type',
                        ['outArchive', 'inArchive', 'contractArchive', 'financeArchive'])->count();
        ...
        $user['certCount'] = Cert::where('citizen_id', $request['subscribe_id'])->where('t_farfromcenter.e_type', '!=',
                4)->count();
        $user['warningCount'] = Cert::where('citizen_id', $request['subscribe_id'])->where('t_farfromcenter.e_type',
                4)->count();

        $user['licArchiveCount'] = count(ArchiveLicense::where('model_id', $request['subscribe_id'])->where('enabled',
                1)
                ->where('model_name', $model)->where('type', 'licArchive')->get());

        $user['licFileArchiveCount'] = count(ArchiveLicense::where('model_id',
                $request['subscribe_id'])->where('enabled', 1)
        ...
        $jalArchiveCount = count(AgendaTopic::where('connected_to', $request['subscribe_id'])
                ->where('model', $model)->get());
        $Archive2 = Archive::where('enabled', 1)
                ->where('type', 'agArchive')
                ->whereHas('relatedTo', function ($query) use ($request) {
                    $query->where('model_id', $request['subscribe_id'])
                            ->where('model_name', 'App\\Models\\User');
                })->count();
        $jalArchiveCount = $jalArchiveCount+$Archive2;
        $user['linkToCount'] +=$Archive2;
        ...
        // }
        $user['EarchLicCount'] = Earh_lic::whereJsonContains('id2', strval($request['subscribe_id']))->where('enabled',
                1)->count();
    ...
    public function subscribe_info(Request $request)
    {
        ...
        return response()->json([
                'status' => false, 'errors' => array('not_allowed' => array('ليس لديك صلاحية لعرض هذا المواطن'))
        ]);
    ...
    public function subscriberCert(Request $request)
    {
        $cert = Cert::select('t_farfromcenter.*', 't_certification.s_name_ar as type_id_name')
                ...
                ->leftJoin('t_certification', 't_certification.pk_i_id',
                        't_farfromcenter.msgTitle')->with('Admin')->orderBy('t_farfromcenter.cer_date', 'DESC')->get();
    ...
    public function subscriberLicArchive(Request $request)
    {
        ...
        $ArchiveLic = ArchiveLicense::where('model_id', $request['subscriber_id'])->where('enabled', 1)
                ->where('model_name', 'App\\Models\\User')->with('License')->select('archive_licenses.*',
    ...
    public function subscriberCopyArchive(Request $request)
    {
        ...
        $Archive = CopyTo::where('model_id', $request['subscriber_id'])->where('enabled', 1)
                ->where('model_name', 'App\\Models\\User')->with('archive', 'archive.files')->with('archive',
                        'archive.Admin')->get();
    ...
    public function subscriberJalArchive(Request $request)
    {
        ...
        $Archive2 = Archive::select('archives.*', 'archives.subject as title', 'archives.decision as descision',
                't_constant.name as type_id_name')
                ->where('enabled', 1)
                ->where('type', 'agArchive')
                ->whereHas('relatedTo', function ($query) use ($request) {
                    $query->where('model_id', $request['subscriber_id'])
                            ->where('model_name', 'App\\Models\\User');
                })
                ->leftJoin('t_constant', 't_constant.id', 'archives.type_id')
                ->with('archiveType', 'files', 'Admin', 'relatedTo')->orderBy('archives.date', 'ASC')->get();
        $Archive = $Archive->mergeRecursive($Archive2)->sortByDesc('date');
        return DataTables::of($Archive)
                ->addIndexColumn()
                ->make(true);
    ...
    public function subscriberEarh_lic(Request $request)
    {
        ...
        $res = Earh_lic::whereJsonContains('id2', strval($request['subscriber_id']))->where('enabled',
                1)->with('Admin')->orderBy('date', 'ASC')->get();
        // dd($res);
        foreach ($res as $row) {
    ...
    public function subscribe_info_all(Request $request)
    {
        ...
        $users = User::select('users.*', 'regions.name as region_name', 'cities.name as city_name',
                ...
                ->leftJoin('towns', 'users.town_id', 'towns.id')->where('users.enabled',
                        '1')->search($request)->orderBy('users.id', 'DESC');
    ...
    function mergeArchive($super, $usersToMerge, Request $request)
    {
        ...
        $archiveLicenses = ArchiveLicense::whereIn('model_id', $request->mergerID)->where('model_name',
                $super->model)->get();
    ...
    function mergeTickets($super, $usersToMerge, Request $request)
    {
        ...
        $AppTicket34s = AppTicket34::whereIn('customer_id', $request->mergerID)->where('customer_model',
                $super->model)->get();
