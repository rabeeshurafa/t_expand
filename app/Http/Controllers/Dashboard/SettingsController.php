    ...
    public function store_JobLic_settings(Request $request){
        ...
        if($setting){
            if ($request->file('header_img_file')) {
                $path1 = $this->upload_image($request->file('header_img_file'), 'setting_');
            }else{
                $path1 = $setting->header_img;
            }
            // if ($request->file('footer_img_file')) {
            //     $path2 = $this->upload_image($request->file('footer_img_file'), 'setting_');
            // }else{
    ...
    public function store_settings(Request $request){
        ...
        if($setting){
            if ($request->file('imgPic')) {
                $path = $this->upload_image($request->file('imgPic'), 'setting_');
            }else{
                $path = $setting->logo;
            }
            if ($request->file('header_img_file')) {
