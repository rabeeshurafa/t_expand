...
use App\Managers\DateOperations;
...
class ProjectController extends Controller
{
    ...
    public function store_project(ProjectRequest $request)
    {
        ...
        $project->dateStart = DateOperations::convertDateForBackend($request->dateStart);
        $project->dateEnd = DateOperations::convertDateForBackend($request->dateEnd);
        $project->contractDate = DateOperations::convertDateForBackend($request->contractDate);
        $project->orderDate = DateOperations::convertDateForBackend($request->orderDate);
        $project->receiveDate = DateOperations::convertDateForBackend($request->receiveDate);
        $project->finishDate = DateOperations::convertDateForBackend($request->finishDate);
        ...
        $project->contractCurrencyID = $request->contractCurrencyID;
        $project->sponser = json_encode($request->orgnizeID);
        $project->contract = json_encode($request->supplierID);
    ...
    public function getProjData(Request $request)
    {
        ...
        $project['info']->dateStart = DateOperations::convertDateForFrontend($project['info']->dateStart);
        $project['info']->dateEnd = DateOperations::convertDateForFrontend($project['info']->dateEnd);
        $project['info']->contractDate = DateOperations::convertDateForFrontend($project['info']->contractDate);
        $project['info']->orderDate = DateOperations::convertDateForFrontend($project['info']->orderDate);
        $project['info']->receiveDate = DateOperations::convertDateForFrontend($project['info']->receiveDate);
        $project['info']->finishDate = DateOperations::convertDateForFrontend($project['info']->finishDate);
    ...
    public function project_info_all(Request $request)
    {
        ...
        if ($request->search_from && $request->search_to) {
            try {
                $from = DateOperations::convertDateForBackend($request->get('search_from'));
                $to = DateOperations::convertDateForBackend($request->get('search_to'));
            } catch (\Exception $e) {
                $from = null;
                $to = null;
            }
        ...
        if ($to != null && $from != null) {
            if (\DateTime::createFromFormat('Y-m-d', $to) !== false && \DateTime::createFromFormat('Y-m-d', $to) !== $from) {
                $project = $project->whereBetween('projects.dateStart', [$from, $to]);
            }
        }
//        ->whereJsonContains('options->languages', 'en')
        if ($request->searchOrganizationId) {
            $project = $project->whereJsonContains('sponser', strval($request->searchOrganizationId));
        }
        if ($request->searchSuppId) {
            $project = $project->whereJsonContains('contract', strval($request->searchSuppId));
        }
