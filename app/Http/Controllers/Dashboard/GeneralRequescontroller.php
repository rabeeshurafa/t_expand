...
class GeneralRequescontroller extends Controller{
    ...
    public function internalMemo()
    {
        ...
        $fees=$this->fees;
        $app_no=44;
        return view('dashboard.generalRequest_Tickts.internalMemo', compact('type','ticketInfo','department','archive_config','app_no','fees','max','positions'));
    }
    ...
    public function trackingArchive($archiveType,$archiveId)
    {
        ...
        $archive_config = ArchiveRole::where('empid', Auth()->user()->id)->where('type', $type)->get();
        $app_no=45;
        return view('dashboard.trackingArchive.index', compact('type','ticketInfo','department','archive_config','app_no','archive'));
    }
}
