...
class DailyWorkController extends Controller
{
    ...
    {
        $request = new Request();
        $request->replace(['from' => $from, 'to' => $to, 'name' => $emp_id, 'mostt' => $mostt, 'state' => $state, 'print' => 1]);
        if ($state == 0) {
            $stateName = 'الكل';
        } else {
        ...
        return view('dashboard.dailyWork.print', compact('works', 'stateName', 'from', 'to','emp_name','beneficialCount'));
    }
