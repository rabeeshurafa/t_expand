...
//        dd($agendaDetail);
        ...
        return view('dashboard.archive.agenda', compact('type', 'archive_type', 'url', 'agendaExtention','agendaDetail'));
    ...
    public function ajaxDisplayMeeting(Request $request)
    {
        ...
        $agendaDetail = AgendaDetail::where('agenda_extention_id', '=', $meetingTitleName)->where('agenda_number', '=', $agendaNum)->where('enabled',1)->get()->first();
        if ($agendaDetail)
            foreach ($agendaDetail->AgendaTopic as $row) {
                if (is_array(json_decode($row->file_ids)))
                    $row->setAttribute('files', File::whereIn('id', json_decode($row->file_ids))->get());
                    ...
                    $row->setAttribute('files', array());
                $row->setAttribute('AgendaTopic', AgendaTopic::where('detail_id', $row->id)->where('enabled',1)->orderBy('id', 'asc')->get());
            }
        else {
            $agendaDetail = array();
        }
    ...
    public function printMeeting($agenda_id)
    {
        ...
        $agendaDetail = AgendaDetail::where('id', '=', $agenda_id)->where('enabled',1)->with('AgendaExtention','AgendaTopic')->first();
        //dd($agendaDetail);
        if ($agendaDetail) {
            foreach ($agendaDetail->AgendaTopic as $row) {
                if (is_array(json_decode($row->file_ids)))
            ...
            $data['employee'] = json_decode($meetingTitleList->employee);
            $empWithPriv = $meetingTitleList->users ? json_decode($meetingTitleList->users):null;
            if (is_array($empWithPriv) && count($empWithPriv) > 0)
                $data['employeeData'] = Admin::whereIn('id', $empWithPriv)->get();
            else
                $data['employeeData'] = array();
