...
class Ticket23TableJob implements ShouldQueue
{
    ...
    public function handle() {
        ...
        // dd($obj);
        $client=new AppTicket23();
        
        $task_type=0;
        if($obj['ticket_name']!=null && $obj['ticket_name']!=''){
            $constant=Constant::where('name',$obj['ticket_name'])->where('parent',6029)->first();
            if($constant!=null){
                $task_type=$constant->id;
            }else{
                $constant=new Constant();
                $constant->name=$obj['ticket_name'];
                $constant->parent=6029;
                $constant->save();
                $task_type=$constant->id;
            }
        }
        ...
        $client['customer_id']          = @$obj['i_ref_id'];
        $client['customer_name']        = @$obj['s_connected_with'];
        $client['customer_mobile']      = @$obj['s_mobile'];
        $client['region']               = 0;
        $client['address']              = '';
        $client['created_at']           = @$obj['dt_created_at'];
        $client['malDesc']              = @$obj['s_content'];
