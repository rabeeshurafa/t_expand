                                            ...
                                            <div class="form-body">
                                                <div class="form-group">
                                                    <div class="input-group" style="width:100%!important">
                                                        ...
                                                        <select id="emp_to_json" name="emp_to_json[]"
                                                                class="select2 form-control" multiple="multiple"
                                                                data-toggle="select" style="width:75%!important">
                                                            @if(isset($employees) && !empty($employees) && count($employees) > 0)
                                                                @foreach ($employees as $key => $value)
                                                                    <option value="{{$value->id}}"
                                                                            {{
                                                                                 in_array($value->id,json_decode(($archive_config->archiveroles??'[]'))) ? "selected" : ""
                                                                            }} >
                                                                        {{$value->nick_name}}
                                                                    </option>
                                                                @endforeach
