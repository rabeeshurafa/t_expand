    ...
    <div class="col-md-6">
        <div class="form-group paddmob">
            <div class="input-group">
                ...
                <select class="form-control networkType" {{ $readonly?"readonly":"" }} name="networkType" id="networkType">
                    <option value="">{{ 'نوع الشبكة' }} </option>
                    @foreach($helpers['networType'] as $networType)
                    <option value="{{ $networType->id }}" {{ $ticket->network_type==$networType->id?'selected':"" }}>{{ $networType->name }}</option>
                    @endforeach
                </select>
...
@include('dashboard.ticketRecive.maldesc',['name_maldesc'=>'وصف الصيانة أو التطوير'])
