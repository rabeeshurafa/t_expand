        ...
        <div class="form-group">
            <div class="input-group" >
                ...
                <select id="type" name="type" type="text"
                    class="form-control valid type" aria-invalid="false" {{ $readonly?"disabled":"" }}>
                    <option value=""> {{ 'اختر' }} </option>
                    <option value="6114" {{ $ticket->complaint_type==6114?'selected':"" }}> {{ 'بدون' }} </option>
                    @foreach($helpers['complaintTypes'] as $complaintType)
                    <option value="{{ $complaintType->id }}" {{ $ticket->complaint_type==$complaintType->id?'selected':"" }}>{{ $complaintType->name }}</option>
                    @endforeach
                </select>
                <div class="input-group-append"onclick="ShowConstantModal(6113,'type','نوع الشكوى')">
                    <span class="input-group-text input-group-text2">
                        <i class="fa fa-external-link"></i>
                    </span>
                </div>
